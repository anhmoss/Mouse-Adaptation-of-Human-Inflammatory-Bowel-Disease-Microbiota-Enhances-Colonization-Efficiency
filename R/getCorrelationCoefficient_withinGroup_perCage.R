...
#metadataNames (metadata column names in addition to the counts/asv names...this is to create a subset of counts table without metadata)
...
getCorrelationCoefficient_withinGroup_perCage = function(tableWithMeta, 
  ...
  for(m in 1:length(phenotypeGroup)){
    ...
    for(n in 1:length(uniqueCages)){
      ...
      df_group_perCage = df_group_perCage_alldf[,!(colnames(df_group_perCage_alldf) %in% metadataNames)]
