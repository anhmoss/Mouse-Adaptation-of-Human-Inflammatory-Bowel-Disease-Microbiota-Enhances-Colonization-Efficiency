## function to generate pcoa plots, flips X axis
## input: counts table, metadata, colors for metadata groups, legendlocation, and plottitle
## output: pcoa plot

generatePCOAPlots_flipXaxis = function(mycountsTable, mymetagroup,mycolors, 
  ...
  a = matrix(c((summary(myPcoa)$site[,1]*(-1)),
               summary(myPcoa)$site[,2]), ncol=2)
  b=plot((summary(myPcoa)$site[,1]*(-1)),summary(myPcoa)$site[,2],
         col=adjustcolor(mycolors[mymetagroup], alpha.f = 0.7),pch=20, cex=2,
         xlim = c(min(summary(myPcoa)$site[,1]*(-1))-0.3, 
                  max(summary(myPcoa)$site[,1]*(-1))+0.3),
         ylim= c(min((summary(myPcoa)$site[,2]))-0.3, 
                 max((summary(myPcoa)$site[,2])+0.3)),
